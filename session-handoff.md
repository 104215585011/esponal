...
### PM 独立验收
- 代码核对：`groupCuesIntoSentences` 使用 `shouldEndTranscriptLine` 标点断句、`MAX_CUES_PER_SENTENCE` 上限强制断句，并跳过空文本。
- 翻译层：按整句调用 `translateSentence`，按 `sentence.text` 缓存，中文展示落在 `translations[sentence.startIndex]`。
- 展示层：西语仍按 cue 渲染并保留查词/seek/高亮；中文按句显示；`ES + 中`、`仅西语`、`仅中文` 三模式均对齐。
- 视觉/运行时：Gemini1 已用真实字幕数据复核，双语 `pl-[42px]` 对齐、仅中文模式时间戳 seek、词汇 hover/LookupCard 均通过。

### 验收中发现并修复
- `tests/ext008.test.mjs` 曾被误改并引入乱码，已回滚；回滚后全量测试仍通过，用户可见源码中文完好。
- `session-handoff.md` 曾出现 Codex2 残缺报告段，已用干净 UTF-8/LF 版本重写顶部验收记录，避免把乱码和断裂 markdown 带入后续交接。
...
**结论**：WATCH-004 验收清单全部满足，`feature_list.json` 已标记 `passing`，关闭。

---

## Ticket: WATCH-004 字幕中文断句优化（句子级合并翻译 + 中文按句显示）
**Time**: 2026-05-31 11:00
**From**: Claude1 (PM)
**To**: Gemini1（设计）→ Codex1（实现）→ Codex2（测试）→ Gemini1（评审）→ Claude1（验收）
**Status**: not_started — 等 Gemini1 出设计稿

**根因**：右侧逐字稿中文断句别扭（半句/残句），因为翻译按字幕 cue 逐块翻，而 YouTube cue 按播放时长切非按句切，一句西语被劈成半句分别送腾讯翻译 → 无上下文硬翻成残句。

**方案**：cue 先用句末标点拼回完整句子再整句翻译；中文展示从「逐 cue 一行」改「逐句一块」。西语展示层不动（逐词查词/高亮/滚动全保留）。翻译调用还更少更省额度。

**完整 ticket**：`docs/tickets/WATCH-004.md`（含句子分组算法要求、翻译层改造、展示层设计要点、验收标准）。

**下一站 Gemini1**：出「中文按句展示」设计稿 → `docs/tickets/WATCH-004-design.md`。需明确中文块位置、ES+中对齐、仅中文模式呈现、字号/颜色（沿用现有中文小一号灰）。

**可并行**：第 1 步「cue→句子分组算法 + 翻译层」无 UI，Codex1 可在等设计稿期间先实现 + 单测。展示层须等设计稿。
