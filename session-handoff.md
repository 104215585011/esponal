...
### PM ????
- ?????`groupCuesIntoSentences` ?? `shouldEndTranscriptLine` ?????`MAX_CUES_PER_SENTENCE` ??????????????
- ????????? `translateSentence`?? `sentence.text` ????????? `translations[sentence.startIndex]`?
- ???????? cue ???????/seek/??????????`ES + ?`?`???`?`???` ???????
- ??/????Gemini1 ????????????? `pl-[42px]` ??????????? seek??? hover/LookupCard ????

### ????????
- `tests/ext008.test.mjs` ????????????????????????????????????
- `session-handoff.md` ??? Codex2 ?????????? UTF-8/LF ??????????????????? markdown ???????

### Verification
- `node --test tests/watch004.test.mjs` -> 3/3 pass
- `npm test` -> 323/323 pass
- `npm run lint:encoding` -> pass

**??**?WATCH-004 ?????????`feature_list.json` ??? `passing`????
