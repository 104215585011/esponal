...
| 2026-06-10 | Codex1 | IMPORT-3/4 production hotfix ready_for_qa: user screenshot showed EPUB production still failing with `/api/import/[id]/epub` 500 and browser direct COS `PUT` 451. Added defensive source-fetch handling to the EPUB API so COS signing/fetch exceptions return diagnostic 502 `source_fetch_failed` rather than naked 500; hardened EPUB parser href lookup by stripping fragment/query suffixes and safely decoding hrefs before ZIP lookup; added small-file same-origin upload fallback `POST /api/import/upload` for COS 451, and updated `uploadImportedDocument()` to try browser direct COS PUT first, then fall back to the server upload route for files <=4MB. Verification: red `node --test tests/import027.test.mjs tests/import028.test.mjs` failed first; green 4/4 pass; focused import upload/reader regression 8/8 pass; wider import regression 24/24 pass; `npx tsc --noEmit --pretty false` pass; `npm run lint:encoding` pass; `npm test` 486/486 pass; `npm run build` pass with existing img/Sentry warnings only. |
| 2026-06-10 | Codex1 | IMPORT-4 COS 451 true fallback ready_for_qa: user screenshot showed `/api/import/upload` still returned 502 because the previous fallback still PUT to the same COS default domain. Added `ImportedDocument.inlineContent Bytes?` plus migration `20260610093500_add_import_inline_content`; changed `POST /api/import/upload` to store small fallback files inline in Postgres with an `inline/...` ossKey marker instead of touching COS; changed `/api/import/[id]/file` and `/api/import/[id]/epub` to serve/parse inline bytes before falling back to COS. Verification: red `node --test tests/import028.test.mjs` failed first; green 2/2 pass; focused import set 7/7 pass; wider import regression 25/25 pass; `npx prisma generate` pass; `npx tsc --noEmit --pretty false` pass; `npm run lint:encoding` pass; `npm test` 487/487 pass; `npm run build` pass with existing img/Sentry warnings only. |
| 2026-06-11 | Codex1 | IMPORT-5 ready_for_qa: restored EPUB image-preserving rendering and split the import reader. `src/lib/import/epub.ts` now returns sanitized chapter HTML with whitelisted tags, inline data-url images, stripped scripts/event handlers, and `data-epub-word` hooks while preserving text fallback. `ImportReaderClient.tsx` is now a thin fullscreen shell; PDF internals moved to `PdfReader.tsx` and EPUB internals moved to `EpubReader.tsx`. PDF byte loading/workerPort/adaptive zoom/text-layer LookupCardStack path preserved. Verification: baseline `npm test` 487/487 pass; red `tests/import029.test.mjs` failed first; `tests/import029.test.mjs` 2/2 pass; import regression slice 29/29 pass; `npx tsc --noEmit --pretty false` pass; `npm run lint:encoding` pass; `npm test` 489/489 pass; `npm run build` pass with existing img/Sentry warnings only. |
| 2026-06-11 | Codex1 | IMPORT-6 ready_for_qa: upgraded the import reader into a real paged reader. Added `src/lib/import/reader-settings.ts` for clamped Aa settings, Spanish sentence splitting, and EPUB sentence wrapping; `ImportReaderClient.tsx` now persists reader settings, exposes Aa/TOC bottom sheets, supports paper/font/line-height/page-turn controls, and stores EPUB progress as `epub:<chapter>:<page>`; `EpubReader.tsx` renders sanitized image-preserving HTML through CSS multi-column pagination; `PdfReader.tsx` keeps pdf.js byte/worker rendering while using a fit-width page strip and sentence-aware lookup; `LookupCard.tsx` adds import-only `朗读整句` through `/api/tts`. Verification: baseline `npm test` 489/489 pass; red `tests/import030.test.mjs` failed first; `tests/import030.test.mjs` 3/3 pass; import regression slice 32/32 pass; `npx tsc --noEmit --pretty false` pass; `npm run lint:encoding` pass; `npm test` 492/492 pass; `npm run build` pass with existing img/Sentry warnings only. |
| 2026-06-11 | Codex1 | MOBILE-R-PERF / INFRA-PW / MOBILE-R-001 ready_for_qa: optimized the watch mobile transcript clock by throttling the `currentTimeSec` path to `TranscriptPanel` and removing mobile row blur; added Playwright mobile+desktop Chromium smoke coverage for public routes, mobile import sheet, console/pageerror guard, and Chinese copy; added an e2e runner watchdog for the current Windows/Next dev-server teardown hang; added the R-001 mobile shell foundation (`routes.ts`, `AppShell`, `useShellBack`) plus placeholder `/home`, `/review`, `/me` pages without migrating business pages. Verification: red focused contracts failed first; focused contracts 8/8 pass; regression slice 35/35 pass; `npx tsc --noEmit --pretty false` pass; `npm run lint:encoding` pass; `npm test` 500/500 pass; `npm run build` pass with existing warnings only; `PLAYWRIGHT_E2E_TIMEOUT_MS=90000 npm run test:e2e -- tests/e2e/smoke.spec.ts` reports 5 pass + 1 desktop mobile-only skip, then watchdog exits after dev-server teardown hang. |
| 2026-06-11 | Codex1 | IMPORT-6/7 hotfix ready_for_qa: split imported reading behavior by format after user feedback. PDF now renders as a WPS-style continuous original preview instead of the old single-page reader frame, with fit-width canvases, scroll-based current page tracking, preserved pdf.js byte/worker path, zoom buttons, progress persistence, and text-layer lookup. EPUB keeps the reader controls but pagination now measures both width and height, considers scrollHeight, listens for image load reflow, and resets page-in-chapter on chapter changes. Verification: red `tests/import031.test.mjs` failed first; import reader regression 22/22 pass; `npx tsc --noEmit --pretty false` pass; `npm run lint:encoding` pass; `npm test` 502/502 pass; `npm run build` pass with existing warnings only. |
| 2026-06-11 | Codex1 | IMPORT-6 reflow anchor hotfix ready_for_qa: fixed PM return item where EPUB font-size/font-family/line-height changes only clamped the numeric page and lost the reading position. `EpubReader.tsx` now captures the first visible sentence/block anchor before pagination-affecting changes and restores to the page containing that same anchor after reflow. Verification: red `tests/import032.test.mjs` failed first; `tests/import030.test.mjs tests/import031.test.mjs tests/import032.test.mjs` 6/6 pass; `npx tsc --noEmit --pretty false` pass; `npm run lint:encoding` pass. |
| 2026-06-11 | Codex1 | IMPORT-7 ready_for_qa: fixed the reader P0 from true-device feedback. PDF now loads through pdf.js URL Range requests against `/api/import/[id]/file` instead of buffering the whole file, the file route supports/forwards 206 partial content headers, PDF sizing uses measured fit-screen scale with lazy page rendering, and EPUB pagination now uses true horizontal CSS columns without locked `pageWidth` content width or `verticalPages` fallback. Verification: red `tests/import033.test.mjs` failed first; import reader focused regression 15/15 pass; `npx tsc --noEmit --pretty false` pass; `npm run lint:encoding` pass; `npm test` 505/505 pass; `npm run build` pass with existing warnings only. |
| 2026-06-11 | Codex1 | IMPORT-7 M1 ready_for_true_device: per PM's staged reset, stopped trying to tune the hand-rolled EPUB reader and replaced the active EPUB runtime with epub.js minimal paginated rendering only. Added `epubjs`; `EpubReader.tsx` now loads `/api/import/[id]/url`, renders the original EPUB via `book.renderTo(... flow: "paginated", spread: "none")`, and maps existing shell prev/next actions to `rendition.prev()` / `rendition.next()`. M2 lookup, M3 themes/TOC/CFI, and M4 PDF are intentionally not started. Verification: red `tests/import034.test.mjs` failed first; focused import reader regression 14/14 pass; `npx tsc --noEmit --pretty false` pass; `npm run lint:encoding` pass; `npm test` 506/506 pass; `npm run build` pass with existing warnings only. |
| 2026-06-11 | Codex1 | IMPORT-7 M1 production hotfix ready_for_true_device: user screenshot showed deployed epub.js stuck on loading with `/api/import/<id>/...` 404s. Root cause: epub.js was given `/api/import/[id]/url`, so internal EPUB relative resources resolved against missing Next routes. `EpubReader.tsx` now fetches authenticated `/api/import/[id]/file`, converts bytes to an `application/epub+zip` Blob URL for epub.js, revokes it on cleanup, and gates ResizeObserver/page turns behind `renditionReady` after `rendition.display()`. Verification: red `node --test tests/import034.test.mjs tests/import027.test.mjs` failed first; focused 4/4 pass; import reader regression 14/14 pass; `npx tsc --noEmit --pretty false` pass; `npm run lint:encoding` pass; `npm test` 506/506 pass; `npm run build` pass with existing warnings only. |
